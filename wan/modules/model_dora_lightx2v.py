...
from .lightx2v_lora_adapter import WanLoraWrapper
...
class WanModel_dora_lightx2v(ModelMixin, ConfigMixin):
    r"""
    ...
    def load_lightx2v_lora(self, lora_path):
        lora_wrapper = WanLoraWrapper(self)
        lora_name = lora_wrapper.load_lora(lora_path)
        lora_wrapper.apply_lora(lora_name, 1.0)
    
    def load_dora(self, dora_path, alpha):
        ...
        state_dict_dora = {}
        with safe_open(dora_path, framework="pt") as f:
            for k in f.keys():
                state_dict_dora[k] = f.get_tensor(k)

        dora_name_dict = get_name_dict(state_dict_dora)
        state_dict_model = self.state_dict()

        for name in dora_name_dict:
            param_info = dora_name_dict[name]
            weight_b = state_dict_dora[param_info["lora_B"]]  
            weight_a = state_dict_dora[param_info["lora_A"]]  
            magnitude = state_dict_dora[param_info["magnitude"]]  
